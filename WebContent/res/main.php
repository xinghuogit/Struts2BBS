...
var ajaxCaches = {};	//不管get还是post一律默认缓存
function ajax(json){
	var url = json.url;
	...
	if(cache){
		if(data in ajaxCaches){
			success(ajaxCaches[data]);	//直接返回缓存数据
			return;
		}
	}
	...
	xmlHttp.open(method, method=='get'?url+'?'+data+'&rand='+Math.random()*100000:url, async);
	if(method=='get'){
		xmlHttp.send(null);
	}else{
